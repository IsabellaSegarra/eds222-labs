...
storm_data <- read_csv(here::here("week 2", "data", "la_storm.csv"))

# Create a plot of the gage height and precipitation data

ggplot(data = storm_data, aes(x = precip_wk_in, y = gage_ht)) +
  geom_point()

  # precip is on the x-axis because precip is the cause/predictor 
```
...
storm_summ <- summary(storm_mod)

# Find the standard deviation (sigma)
print(storm_summ$sigma) # Since it is 2.3, 95% of the data should fall within 4.6 points (1-2 standard deviations)

# Find the R squared 
print(storm_summ$r.squared)

# Find the coefficients 
print(storm_summ$coefficients)

# Pull out the intercept/B

print(storm_summ$coefficients[1,1])
```

**Q2: What are the estimates for** $\hat \beta_0, \hat \beta_1, \hat \sigma$ **?**

beta 0 hat is 3.9 which is the y-intercept. If there is no rain, then the gage is at 3.9 ht.

beta 1 hat 2.2: is the rate of precipitation; for every inch of rain, the increase in gage height.
...
beta_0 hat: the y-intercept. If there is no rain, then the gage is at 3.9 ht.

beta 1 hat: is the rate of precipitation; for every inch of rain, the increase in gage height.

sigma hat: the variance
...
# Different components needed to get R-squared 

y_bar <- mean(storm_data$gage_ht) # The overall mean
x <- storm_data$precip_wk_in # the x values of all the points
y_hat <- 3.9 + 2.2 * x
SST <- sum((storm_data$gage_ht - y_bar)^2)
SSE <- sum((storm_data$gage_ht - y_hat)^2)

# What fraction of the variance in gage height have we explained using precipitation? 
...
y: this is the response variable (observed gage heights)

y bar: mean gage height (overall)

y hat: expected gage height (given precip)

**Q5: What does the** $R^2$ **value tell you about the quality of the model (in plain language)?**

Having precipitation reduces the error of our predictions by about 55%.
...
mu <- beta_0 + beta_1 * x  #y hat = mu
y <- rnorm(n = 100, mean = mu, sd = sigma) # Plut into rnorm function because the data is normally distributed. 
...
sim_dat <- tibble(x,y)
sim_mod <- lm(y ~ x, data = sim_dat) # Fit y with x as the predictor
...
beta_0_hat <- coef(sim_mod)[1] # Can also do it this way: mod_summ$coefficients[2,1]
beta_1_hat <- coef(sim_mod)[2] # Can also do it this way: mod_summ$coefficients["x", "Estimate"]
...
rreas_murres <- read_csv(here::here("week 2", "data/rreas.csv"))

glimpse(rreas_murres)
...
# fish abundance index = a relative, standardized measurement of a fish population's size or biomass over time 

ggplot(rreas_murres, aes(x= rreas_rockfish_index, y = comu_rockfish_diet)) +
  geom_point() +
  coord_flip()
...
I chose the rockfish abundance as the predictor variable for the diet because the abundance of fish influences the available food for their diet.
...
rockfish_mod <- lm(rreas_rockfish_index ~ comu_rockfish_diet, data = rreas_murres)

summary(rockfish_mod)
...
y_bar <- mean(rreas_murres$) # The overall mean
x <- rreas_murres$precip_wk_in # the x values of all the points
y_hat <- 3.9 + 2.2 * x
SST <- sum((storm_data$gage_ht - y_bar)^2)
SSE <- sum((storm_data$gage_ht - y_hat)^2)
