...
restoration <- read_csv("week 4/eelgrass.csv",
...
The p-value is larger than the critical threshold, therefore we *fail* to reject the null hypothesis. The data is not consistent with the hypothesis that the popsicle stick method leads to more successful treatments.
...
# --QUESTION: why mean for p hat?--

# Calculate the proportions of the two treatments
# Let treatment 1 be "Garden staple"
p_hat1 <- mean(restoration$success[restoration$treatment == "Garden staple"]) # pulling out just the success from the treatment column for garden staples 
...
**Q10: How would you interpret this confidence interval** We are 95% confident the true difference in proportions falls between -0.06 and 0.34.
