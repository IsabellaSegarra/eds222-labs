...
```{r}
#| label: q3
# Calculating the difference of proportions between 

# Count within the 'success_fct' column the number of "succeed" and "fail"
              
props <- restoration %>% 
  group_by(treatment) %>%  # Group by 
  summarize(proportion = sum(success), 
            treat_prob = proportion/n())

props

# Difference of proportions between gs and ps (your HA is better)
props$treat_prob[2] - props$treat_prob[1]
...
test_stat <- restoration_summary$prop_success[2] - restoration_summary$prop_success[1]
...
ggplot() +
  geom_histogram(aes(null_distribution)) + 
geom_vline(xintercept = test_stat) # Add a vertical line 
...
p_val <- sum(null_distribution > test_stat) / length(null_distribution)
p_val
