...
restoration <- read_csv(here::here("week 4", "eelgrass.csv"),
...
**Q1: What is the appropriate test statistic for this question?** The test statistic is a difference in proportions.

### State your null and alternative hypotheses

**Q2: What are your null and alternative hypotheses?**

H0:There is no effect on restoration between the two restoration methods.

HA: The popsicle stick method has a better effect on restoration.
...
diff_props <- restoration_summary$prop_success[2] - restoration_summary$prop_success[1]
...
**Q4: What column should we shuffle to break the association between treatment and outcome?** Shuffle by 'treatment' column.
...
# One perumutation = one shuffle with the sample() function

# shuffling the treatment column
one_permutation <- restoration %>%
  mutate(treatment = sample(treatment, size = length(treatment), replace = FALSE)) #sample the length of treatment 

# Look at the prop_success for the one permutation
permutation_props <- one_permutation %>%
  group_by(treatment) %>%
  summarize(prop_success = sum(success / n()))

# Get the sample statitisc for the one permutation
...
# Function called 'permute' to build null distribution 
# Apply the one permutation for 1000s of permutations
...
# (1:1000) = samples 
null_distribution <- map_dbl(1:1000, permute) # this is the function itself,assuming there  
...
null_distribution_df <- tibble(null_distribution) # Make null dist. into a tibble/df
ggplot(null_distribution_df, aes(null_distribution)) +
  geom_histogram(binwidth = 0.05) +
  geom_vline(xintercept = diff_props, # x-int = diff_props/stat
             color = "firebrick",
             linewidth = 1.5)
```
...
one_bootstrap <- restoration %>% 
  group_by(treatment) %>%  #group by treatment b/c it matters for this test
...
Estimate the test status of the bootstrap distribution
...
```{r}
#| label: q15
# Visualize the diff_prop column that was made previously
...
bootstrap_plot <- ggplot(boot_diff_prop, aes(diff_prop)) +
  geom_histogram() +
